            ...
            <div class="modal-body">
                ...
                <form action="{{ route('ajaxImageUpload') }}" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="form-group">
                        <input type="text" class="form-control" name="title" class="require" required id="title"
                               placeholder="{{awtTrans('title','ar')}}">
                    </div>
                    <div class="form-group">
                    ...
                    <p class="text-center">
                        <button class="btn btn-primary upload-image" type="submit"><i class="fa fa-plus"
                                                                                      aria-hidden="true"></i>
                            ...
                            @foreach($Post_draftsman as $post)
                                @foreach($post->file as $file)
                                    <tr class="{{'portfolioimg'.$file->id}}">
                                        <td><img src="{{Storage::url($file->full_path)}}"
                                                 class="img-responsive img-rounded" style="width: 80px" alt=""></td>
                                        <td>{{$post->title}}</td>
                                        <td>
